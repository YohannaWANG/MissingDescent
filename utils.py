...
def convert_file_into_csv():
    """
    Convert .xls file into csv file
    Returns: file.csv
    """
    read_file = pd.read_excel (r'uci_data/Concrete_Data.xls')
    read_file.to_csv(r'uci_data/Concrete_Data.csv', index = None, header=True)
...
def produce_NA(X, p_miss, mecha="MCAR", opt=None, p_obs=None, q=None):
    """
    Code from https://rmisstastic.netlify.app/how-to/python/generate_html/how%20to%20generate%20missing%20values
...
def missing_examples():
    ...
    X_complete_cont, N, d, mu, Sigma = synthetic_data()
    ...
    plt.title('Self-masking MANR')
    plt.show()

    # MANR with quantile censorship
    # Sample mnar missing data with quantiles -----------------------------------------
    X_miss_quant = produce_NA(X_complete_cont, p_miss=0.4, mecha="MNAR", opt="quantile", p_obs=0.5, q=0.5)

    X_mnar_quant = X_miss_quant['X_incomp']
    R_mnar_quant = X_miss_quant['mask']

    print("Percentage of generated missing values in quantile: ", (R_mnar_quant.sum()).numpy()/np.prod(R_mnar_quant.size())*100, " %")

    ax = sns.heatmap(X_mnar_quant, mask=R_mnar_quant.numpy()==1, linewidths=0.005, linecolor='black')
    plt.title('Quantile MNAR')
    plt.show()
